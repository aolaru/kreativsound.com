...
for (const page of pages) {
  ...
  assert(html.includes("v0.4.7"), `${page.name}: release version should be v0.4.7`);
...
assert(changelogHtml.includes("v0.4.7"), "Changelog: current version is missing");
...
assert(scratchPack.every((preset) => preset.parameterMap.delay_feedback <= 0.56), "Scratch: delay feedback should respect family limits");
assert(
...
assert(audioPack.every((preset) => preset.parameterMap.delay_feedback <= 0.56), "Audio: delay feedback should respect family limits");

const shortAudioProfile = buildAudioProfile({ centroidHz: 2400, pitchHz: 146, onsetRatio: 0.22, sustainRatio: 0.58, movement: 0.32, flatness: 0.18, zeroCrossRate: 0.04, stereoWidth: 0.5, duration: 1.2 });
const longAudioProfile = buildAudioProfile({ centroidHz: 2400, pitchHz: 146, onsetRatio: 0.22, sustainRatio: 0.58, movement: 0.32, flatness: 0.18, zeroCrossRate: 0.04, stereoWidth: 0.5, duration: 12 });
assert(longAudioProfile.sustain > shortAudioProfile.sustain && longAudioProfile.wash > shortAudioProfile.wash, "Audio: duration should influence sustained space decisions");
